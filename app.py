...
class CourseSearchApp:
    ...
    <p class='caption-style' style='font-size: 24px; color: black;'>Ask me anything about courses at <span style='color: {duke_blue};'>Duke University</span>.</p>
    """, unsafe_allow_html=True)
        ...
        prompt = st.text_input("What are you looking for?", value=st.session_state['current_input'], placeholder="Search...", key="new")
        ...
        for answer in st.session_state['displayed_answers']:
            st.markdown(f'<div style="color: {duke_blue};"><b>Discovery: </b>{answer}</div>', unsafe_allow_html=True)
                ...
                if event.data.object == 'thread.message.delta':
                    for content in event.data.delta.content:
                        if content.type == 'text' and content.text.value is not None:
                            ...
                            # Prepend "Discovery:" in bold and blue to the streamed content
                            res_box.markdown(f'<div style="color: {duke_blue};"><b>Discovery:</b> {cleaned_result}</div>', unsafe_allow_html=True)
                            output = result
